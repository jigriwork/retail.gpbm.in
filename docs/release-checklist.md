...
The complete combined-history response also required **migration 12**, `20260910150000_native_json_analytics_response.sql`, after migration 11. It assembles native JSON with `row_to_json`/`json_agg` rather than repeatedly copying binary JSON. The RPC parameters and HTTP JSON shape are unchanged; its SQL scalar return type changes from JSONB to JSON. The function has no database dependants (verified before release); its replacement is transactional, uses DROP RESTRICT, restores the same grants and requests a PostgREST schema reload. No historical data or table is changed. The backup rehearsal must compare the complete normalized response fingerprints, not only totals, and actual PostgREST must verify more than 1,000 distinct result groups. Do not undo this with a destructive data rollback.
