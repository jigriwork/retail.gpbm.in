-- Assemble the HTTP JSON response without workbook-sized binary JSON copies.
-- JSON object shape, parameters, grants, dimensions and totals are unchanged.
-- No database objects depend on this RPC; DROP is RESTRICT and transactional.
begin;
drop function public.analytics_data(uuid[],date,date,date[]);
...
declare result json; begin
 ...
 with sales as (
  ...
  where r.store_id=any(p_store_ids) and r.sale_date between p_start and p_end
    and p.status='processed' and p.is_current
  ...
  'sales',coalesce((select json_agg(row_to_json(s) order by store_id,sale_date,bill_no,item_name) from sales s),'[]'),
  'stock',coalesce((select json_agg(row_to_json(s) order by store_id,stock_month,item_name) from stock s),'[]'),
  'aliases',coalesce((select json_agg(json_build_object('store_id',store_id,'normalized_source_name',normalized_source_name,'canonical_staff_name',canonical_staff_name)) from public.staff_name_aliases where store_id=any(p_store_ids) and source_type='sales_report' and is_active=true),'[]'),
...
grant execute on function public.analytics_data(uuid[],date,date,date[]) to authenticated,service_role;
